...
import { Country, PhoneNumberSelectorWithFlagsProps } from '../types';

const PhoneNumberSelectorWithFlags: React.FC<PhoneNumberSelectorWithFlagsProps> = () => {
  ...
  useEffect(() => {
    setSelectedFlag(countryList.find((country) => country.name === 'Bermuda'));
    setSelectedPhoneCode(countryList.find((country) => country.name === 'Bermuda')?.code);
  }, []);
