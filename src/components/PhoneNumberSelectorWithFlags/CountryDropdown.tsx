...
const CountryDropdown: React.FC<CountryDropdownProps> = ({ setSelectedPhoneCode, setSelectedFlag, setOpen, numberInputerRef, searchInputerRef, open }) => {
  const [searchValue, setSearchValue] = useState<string>('');
  const countries = countryList.filter((country) => country.name.toLowerCase().includes(searchValue));
  ...
  return (
    <>
      <input type="text" placeholder="Search" onChange={handleSearch} value={searchValue} aria-label="Search Country Name" onKeyDown={handleKeyDown} autoComplete="off" ref={searchInputerRef} />
      <div className={`${styles.dropdown}`} ref={scrollRef}>
        <ul>
